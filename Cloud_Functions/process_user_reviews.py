...
try:
    ...
    print(f"BigQuery client initialized for User_Reviews: {PROJECT_ID}.{DATASET_ID}.{TABLE_ID}")
except FileNotFoundError:
    print(f"ERROR: Service account key file not found at '{SERVICE_ACCOUNT_KEY_PATH}'. Ensure it's in backend/.")
    client = None
    table_ref = None
except Exception as e:
    print(f"ERROR: Failed to initialize BigQuery client for User_Reviews: {e}")
    client = None
    table_ref = None

@functions_framework.http
def process_user_reviews(request): # <--- UNIQUE FUNCTION NAME
    """
    Cloud Function to handle 'User_Reviews' events (e.g., submitting a review).
    """
    print(f"[{datetime.datetime.now(datetime.timezone.utc).isoformat()}] User_Reviews Request received. Method: {request.method}")
    ...
    if request.method == 'POST':
        ...
        if event_name != 'User_Reviews': # <--- Specific check for this function
            print(f"Error: Mismatched event name '{event_name}'. Expected 'User_Reviews'.")
            return json.dumps({"status": "error", "message": f"Expected 'User_Reviews' event, got '{event_name}'"}), 400, response_headers
        ...
        # Specific parameters for User_Reviews, now nested under 'review'
        ...
        row_to_insert = {
            ...
            "review": { # Now using the nested 'review' field
                "product_id": review_data.get('product_id'),
                "review_text": review_data.get('review_text'),
                "rating": review_data.get('rating')
                # review_id is not in your provided 'review' schema, if needed, add it back to schema.
            }
        }
