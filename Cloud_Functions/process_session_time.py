...
TABLE_ID = os.environ.get('TABLE_ID', 'session_time')
...
# --- Cloud Function: Session Time Event Tracker ---
@functions_framework.http
def process_session_time_event(request):
    """
    Cloud Function to handle 'session_time' events and insert them into the
    'session_time' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        # Validate that the event is a 'session_time' event
        if 'event_name' not in event_data or event_data['event_name'] != 'session_time':
            print(f"Event name mismatch: Expected 'session_time', got '{event_data.get('event_name')}'.")
        ...
        # Ensure session_duration is a valid integer before inserting
        if row_to_insert.get('session_duration') is not None:
            try:
                row_to_insert['session_duration'] = int(row_to_insert['session_duration'])
            except (ValueError, TypeError):
                print(f"WARNING: session_duration '{row_to_insert['session_duration']}' is not a valid integer. Defaulting to None.")
                row_to_insert['session_duration'] = None
