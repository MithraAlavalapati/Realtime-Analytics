...
TABLE_ID = os.environ.get('TABLE_ID', 'view_item')
...
# --- Cloud Function: View Item Event Tracker ---
@functions_framework.http
def process_view_item_event(request):
    """
    Cloud Function to handle 'view_item' events and insert them into the
    'view_item' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        # Validate that the event is a 'view_item' event
        if 'event_name' not in event_data or event_data['event_name'] != 'view_item':
            print(f"Event name mismatch: Expected 'view_item', got '{event_data.get('event_name')}'.")
