...
TABLE_ID = os.environ.get('TABLE_ID', 'zooming')
...
# --- Cloud Function: Zooming Event Tracker ---
@functions_framework.http
def process_zooming_event(request):
    """
    Cloud Function to handle 'zooming' events and insert them into the
    'zooming' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        # Validate that the event is a 'zooming' event
        if 'event_name' not in event_data or event_data['event_name'] != 'zooming':
            print(f"Event name mismatch: Expected 'zooming', got '{event_data.get('event_name')}'.")
        ...
        # Ensure zoom_level is a valid float before inserting
        zoom_level = row_to_insert.get('zoom_level')
        if zoom_level is not None:
            try:
                row_to_insert['zoom_level'] = float(zoom_level)
            except (ValueError, TypeError):
                print(f"WARNING: zoom_level '{zoom_level}' is not a valid float. Defaulting to None.")
                row_to_insert['zoom_level'] = None
