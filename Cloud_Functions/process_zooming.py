...
try:
    ...
    print(f"BigQuery client initialized for Zooming: {PROJECT_ID}.{DATASET_ID}.{TABLE_ID}")
except FileNotFoundError:
    print(f"ERROR: Service account key file not found at '{SERVICE_ACCOUNT_KEY_PATH}'. Ensure it's in backend/.")
    client = None
    table_ref = None
except Exception as e:
    print(f"ERROR: Failed to initialize BigQuery client for Zooming: {e}")
    client = None
    table_ref = None

@functions_framework.http
def process_zooming(request): # <--- UNIQUE FUNCTION NAME
    """
    Cloud Function to handle 'Zooming' events.
    """
    print(f"[{datetime.datetime.now(datetime.timezone.utc).isoformat()}] Zooming Request received. Method: {request.method}")
    ...
    if request.method == 'POST':
        ...
        if event_name != 'Zooming': # <--- Specific check for this function
            print(f"Error: Mismatched event name '{event_name}'. Expected 'Zooming'.")
            return json.dumps({"status": "error", "message": f"Expected 'Zooming' event, got '{event_name}'"}), 400, response_headers
        ...
        # Specific parameters for Zooming
        zoom_level = request_json.get('zoom_level')
        # zoom_target field from previous implementation is not in new schema's top-level or sub-records.
        # If you need it, you might consider adding it to the BQ schema or 'item' record if relevant.
        ...
        row_to_insert = {
            ...
            "zoom_level": zoom_level # Specific to zooming event
            # "zoom_target" is removed as per new schema
        }
