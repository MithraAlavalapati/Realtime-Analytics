...
# --- Cloud Function: Session Time Tracker ---
@functions_framework.http
@cross_origin()
def process_session_time_event(request):
    """
    Processes a session time event from an HTTP request.
    Initializes BigQuery and Pub/Sub clients within the function scope to prevent fork() errors.
    """
    ...
    EVENT_TABLE_ID = os.environ.get('BIGQUERY_TABLE_ID_SESSION_TIME', 'session_time')
    ...
    try:
        ...
        if event_data.get('event_name') != 'session_time':
        ...
        # --- UPDATED: Publish a message to the notification topic if seller_id exists ---
        seller_id = event_data.get('seller_id')
        store_name = event_data.get('store_name')
        session_duration = event_data.get('session_duration', 'unknown')
        if seller_id and store_name:
            # Create a user-friendly message for the UI
            notification_message = f"A customer spent {session_duration} seconds in your store: {store_name}"
            
            # The notification payload now includes the full log data
        ...
        print(f"Successfully inserted 'session_time' event for user '{event_data.get('user_id')}'.")
