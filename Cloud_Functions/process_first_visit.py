...
TABLE_ID = os.environ.get('TABLE_ID', 'first_visit')
...
# --- Cloud Function: First Visit Event Tracker ---
@functions_framework.http
def process_first_visit_event(request):
    """
    Cloud Function to handle 'first_visit' events and insert them into the
    'first_visit' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        # Validate that the event is a 'first_visit' event
        if 'event_name' not in event_data or event_data['event_name'] != 'first_visit':
            print(f"Event name mismatch: Expected 'first_visit', got '{event_data.get('event_name')}'.")
