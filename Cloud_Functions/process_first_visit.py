...
try:
    ...
    print(f"BigQuery client initialized for first_visit: {PROJECT_ID}.{DATASET_ID}.{TABLE_ID}")
except FileNotFoundError:
    print(f"ERROR: Service account key file not found at '{SERVICE_ACCOUNT_KEY_PATH}'. Ensure it's in backend/.")
    client = None
    table_ref = None
except Exception as e:
    print(f"ERROR: Failed to initialize BigQuery client for first_visit: {e}")
    client = None
    table_ref = None

@functions_framework.http
def process_first_visit(request): # <--- UNIQUE FUNCTION NAME
    """
    Cloud Function to handle 'first_visit' events.
    """
    print(f"[{datetime.datetime.now(datetime.timezone.utc).isoformat()}] first_visit Request received. Method: {request.method}")
    ...
    if request.method == 'POST':
        ...
        if event_name != 'first_visit': # <--- Specific check for this function
            print(f"Error: Mismatched event name '{event_name}'. Expected 'first_visit'.")
            return json.dumps({"status": "error", "message": f"Expected 'first_visit' event, got '{event_name}'"}), 400, response_headers
