...
TABLE_ID = os.environ.get('TABLE_ID', 'view_promotion')
...
# --- Cloud Function: View Promotion Event Tracker ---
@functions_framework.http
def process_view_promotion_event(request):
    """
    Cloud Function to handle 'view_promotion' events and insert them into the
    'view_promotion' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        # Validate that the event is a 'view_promotion' event
        if 'event_name' not in event_data or event_data['event_name'] != 'view_promotion':
            print(f"Event name mismatch: Expected 'view_promotion', got '{event_data.get('event_name')}'.")
