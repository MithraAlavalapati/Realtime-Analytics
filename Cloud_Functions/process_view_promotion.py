...
try:
    ...
    print(f"BigQuery client initialized for view_promotion: {PROJECT_ID}.{DATASET_ID}.{TABLE_ID}")
except FileNotFoundError:
    print(f"ERROR: Service account key file not found at '{SERVICE_ACCOUNT_KEY_PATH}'. Ensure it's in backend/.")
    client = None
    table_ref = None
except Exception as e:
    print(f"ERROR: Failed to initialize BigQuery client for view_promotion: {e}")
    client = None
    table_ref = None

@functions_framework.http
def process_view_promotion(request): # <--- UNIQUE FUNCTION NAME
    """
    Cloud Function to handle 'view_promotion' events.
    """
    print(f"[{datetime.datetime.now(datetime.timezone.utc).isoformat()}] view_promotion Request received. Method: {request.method}")
    ...
    if request.method == 'POST':
        ...
        if event_name != 'view_promotion': # <--- Specific check for this function
            print(f"Error: Mismatched event name '{event_name}'. Expected 'view_promotion'.")
            return json.dumps({"status": "error", "message": f"Expected 'view_promotion' event, got '{event_name}'"}), 400, response_headers
        ...
        # Specific parameters for view_promotion, now nested under 'promotion'
        ...
        row_to_insert = {
            ...
            "promotion": { # Now using the nested 'promotion' field
