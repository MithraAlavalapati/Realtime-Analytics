...
# --- Cloud Function: View User Reviews Tracker ---
...
def process_view_user_reviews_event(request):
    ...
    EVENT_TABLE_ID = os.environ.get('BIGQUERY_TABLE_ID_VIEW_USER_REVIEWS', 'view_user_reviews')
    ...
    try:
        ...
        if event_data.get('event_name') != 'view_user_reviews':
        ...
        if seller_id and store_name:
            notification_payload = {
                ...
                "event_name": event_data.get('event_name'),
                "item_id": event_data.get('item', {}).get('item_id'),
                "message": f"A customer viewed reviews for your product: {event_data.get('item', {}).get('item_name')}"
            }
        ...
        print(f"Successfully inserted 'view_user_reviews' event for user '{event_data.get('user_id')}'.")
