# process_view_product_details.py
...
try:
    ...
    print(f"BigQuery client initialized for View_Product_Details: {PROJECT_ID}.{DATASET_ID}.{TABLE_ID}")
except FileNotFoundError:
    print(f"ERROR: Service account key file not found at '{SERVICE_ACCOUNT_KEY_PATH}'. Ensure it's in backend/.")
    client = None
    table_ref = None
except Exception as e:
    print(f"ERROR: Failed to initialize BigQuery client for View_Product_Details: {e}")
    client = None
    table_ref = None

@functions_framework.http
def process_view_product_details(request): # <--- UNIQUE FUNCTION NAME
    """
    Cloud Function to handle 'View_Product_Details' events.
    """
    print(f"[{datetime.datetime.now(datetime.timezone.utc).isoformat()}] View_Product_Details Request received. Method: {request.method}")
    ...
    if request.method == 'POST':
        ...
        if event_name != 'View_Product_Details': # <--- Specific check for this function
            print(f"Error: Mismatched event name '{event_name}'. Expected 'View_Product_Details'.")
            return json.dumps({"status": "error", "message": f"Expected 'View_Product_Details' event, got '{event_name}'"}), 400, response_headers
        ...
        # Specific parameters for View_Product_Details, now nested under 'item'
        ...
        row_to_insert = {
            ...
            "item": { # Now using the nested 'item' field
                "item_id": item_data.get('item_id'),
                "item_name": item_data.get('item_name'),
                "price": item_data.get('price'),
                "item_category": item_data.get('item_category'),
                "item_brand": item_data.get('item_brand'),
                "item_varient": item_data.get('item_varient') # Make sure this matches frontend casing
            }
        }
