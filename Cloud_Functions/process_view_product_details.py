...
TABLE_ID = os.environ.get('TABLE_ID', 'view_product_details')
...
# --- Cloud Function: View Product Details Event Tracker ---
@functions_framework.http
def process_view_product_details_event(request):
    """
    Cloud Function to handle 'view_product_details' events and insert them into the
    'view_product_details' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        # Validate that the event is a 'view_product_details' event
        if 'event_name' not in event_data or event_data['event_name'] != 'view_product_details':
            print(f"Event name mismatch: Expected 'view_product_details', got '{event_data.get('event_name')}'.")
        ...
        # Ensure price is a valid float before inserting
