...
TABLE_ID = os.environ.get('TABLE_ID', 'page_view')
...
# --- Cloud Function: View Page Event Tracker ---
@functions_framework.http
def process_view_page_event(request):
    """
    Cloud Function to handle 'view_page' events and insert them into the
    'view_page' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        # Validate that the event is a 'view_page' event
        if 'event_name' not in event_data or event_data['event_name'] != 'view_page':
            print(f"Event name mismatch: Expected 'view_page', got '{event_data.get('event_name')}'.")
