...
TABLE_ID = os.environ.get('TABLE_ID', 'view_user_reviews')
...
# --- Cloud Function: View User Reviews Event Tracker ---
@functions_framework.http
def process_view_user_reviews_event(request):
    """
    Cloud Function to handle 'view_user_reviews' events and insert them into the
    'view_user_reviews' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        # Validate that the event is a 'view_user_reviews' event
        if 'event_name' not in event_data or event_data['event_name'] != 'view_user_reviews':
            print(f"Event name mismatch: Expected 'view_user_reviews', got '{event_data.get('event_name')}'.")
        ...
        # Ensure integer and float data types are correct
        for field in ['viewed_reviews_count']:
            value = row_to_insert.get(field)
            if value is not None:
                try:
                    row_to_insert[field] = int(value)
                except (ValueError, TypeError):
                    print(f"WARNING: Field '{field}' with value '{value}' is not a valid integer. Defaulting to None.")
                    row_to_insert[field] = None
        
        item_price = row_to_insert.get('item', {}).get('price')
        if item_price is not None:
            try:
                row_to_insert['item']['price'] = float(item_price)
            except (ValueError, TypeError):
                print(f"WARNING: Item price '{item_price}' is not a valid float. Defaulting to None.")
                row_to_insert['item']['price'] = None
