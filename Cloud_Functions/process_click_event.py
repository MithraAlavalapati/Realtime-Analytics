...
TABLE_ID = os.environ.get('TABLE_ID', 'click')
...
# --- Cloud Function: Click Event Tracker ---
@functions_framework.http
def process_click_event(request):
    """
    Cloud Function to handle 'click' events and insert them into the
    'click' BigQuery table using streaming inserts.
    """
    ...
    try:
        ...
        if 'event_name' not in event_data or event_data['event_name'] != 'click':
            print(f"Event name mismatch: Expected 'click', got '{event_data.get('event_name')}'.")
            return json.dumps({"status": "error", "message": "Event name mismatch."}), 400, {'Access-Control-Allow-Origin': '*'}
        
        # --- FIX: Parse and reformat timestamps ---
        # Get the event_timestamp from the incoming payload
        event_timestamp_str = event_data.get('event_timestamp', datetime.datetime.now(datetime.timezone.utc).isoformat())
        
        # Parse the incoming timestamp and reformat it for BigQuery's strict parser
        try:
        ...
        # Do the same for ingestion_timestamp
