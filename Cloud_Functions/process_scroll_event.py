...
try:
    ...
    print(f"BigQuery client initialized for scroll: {PROJECT_ID}.{DATASET_ID}.{TABLE_ID}")
except FileNotFoundError:
    print(f"ERROR: Service account key file not found at '{SERVICE_ACCOUNT_KEY_PATH}'. Ensure it's in backend/.")
    client = None
    table_ref = None
except Exception as e:
    print(f"ERROR: Failed to initialize BigQuery client for scroll: {e}")
    client = None
    table_ref = None

@functions_framework.http
def process_scroll_event(request): # <--- UNIQUE FUNCTION NAME
    """
    Cloud Function to handle 'scroll' events.
    """
    print(f"[{datetime.datetime.now(datetime.timezone.utc).isoformat()}] scroll Request received. Method: {request.method}")
    ...
    if request.method == 'POST':
        ...
        if event_name != 'scroll': # <--- Specific check for this function
            print(f"Error: Mismatched event name '{event_name}'. Expected 'scroll'.")
            return json.dumps({"status": "error", "message": f"Expected 'scroll' event, got '{event_name}'"}), 400, response_headers
        ...
        row_to_insert = {
            ...
            "scroll_depth_percentage": scroll_depth_percentage # Specific to scroll event
        }
