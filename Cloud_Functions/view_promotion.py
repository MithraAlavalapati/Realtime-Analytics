...
EVENT_TABLE_ID = os.environ.get('BIGQUERY_TABLE_ID_VIEW_PROMOTION', 'view_promotion')
...
try:
    ...
    print(f"DEBUG: BigQuery and Pub/Sub clients initialized.")
except Exception as e:
    client = None
    event_table_ref = None
    pubsub_publisher = None

# --- Cloud Function: View Promotion Tracker ---
@functions_framework.http
def process_view_promotion_event(request):
    response_headers = {
    ...
    try:
        ...
        required_promotion = ['promotion_id']
        ...
        if event_data.get('event_name') != 'view_promotion':
            return json.dumps({"status": "error", "message": "Event name mismatch."}), 400, response_headers
        if not all(field in event_data.get('promotion', {}) for field in required_promotion):
            return json.dumps({"status": "error", "message": "Missing required 'promotion' fields."}), 400, response_headers
        ...
        row_to_insert = {
            ...
            "promotion": {
                "promotion_id": event_data.get('promotion', {}).get('promotion_id'),
                "promotion_name": event_data.get('promotion', {}).get('promotion_name'),
                "creative_name": event_data.get('promotion', {}).get('creative_name')
            } if event_data.get('promotion') else None,
            "device": {
        ...
        if seller_id and store_name:
            notification_payload = {
                ...
                "event_name": event_data.get('event_name'),
                "message": f"A customer viewed a promotion for your store: {store_name}"
            }
        ...
        print(f"Successfully inserted 'view_promotion' event for user '{event_data.get('user_id')}'.")
