...
# --- Cloud Function: View Product Tracker ---
@functions_framework.http
@cross_origin()
def process_view_product_event(request):
    """
    Processes a view product event from an HTTP request.
    ...
    # --- CONFIGURATION (can remain here or be moved inside if dynamic per request) ---
    PROJECT_ID = os.environ.get('GCP_PROJECT_ID', 'svaraflow')
    DATASET_ID = os.environ.get('BIGQUERY_DATASET_ID', 'seller1_data')
    EVENT_TABLE_ID = os.environ.get('BIGQUERY_TABLE_ID_VIEW_PRODUCT', 'view_product')
    NOTIFICATION_TOPIC_ID = 'seller-notifications-topic'
    SERVICE_ACCOUNT_KEY_PATH = "key.json"

    # --- Initialize Clients with key.json credentials INSIDE THE FUNCTION ---
    client = None
    ...
    try:
        ...
        # Return an error response if clients could not be initialized
    ...
    # Check if clients are ready after initialization attempt
    if client is None or event_table_ref is None or pubsub_publisher is None:
        return json.dumps({"status": "error", "message": "BigQuery or Pub/Sub client not ready after initialization."}), 500, response_headers
    ...
    try:
        ...
        required_top_level = ['event_name', 'event_timestamp', 'session_id']
        ...
        if event_data.get('event_name') != 'view_product':
        ...
        seller_id = event_data.get('seller_id')
        store_name = event_data.get('store_name', 'N/A')
        if seller_id and store_name:
            notification_payload = {
                "seller_id": seller_id,
                "event_name": "view_product",
                "item_id": event_data.get('item', {}).get('item_id'),
                "store_name": store_name, # Pass store_name explicitly
                "message": f"A customer just viewed your product: {event_data.get('item', {}).get('item_name')}"
            }
        ...
        print(f"Successfully inserted 'view_product' event for item '{event_data.get('item', {}).get('item_id')}'.")
